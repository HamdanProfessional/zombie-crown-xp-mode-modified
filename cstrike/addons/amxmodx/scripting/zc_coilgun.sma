...
#define SUBMODEL 14
#define MODEL_V "models/zombie_crown/v_coilmg.mdl"
#define MODEL_P "models/p_m249.mdl"
#define MODEL_W "models/zombie_crown/w_primary.mdl"
...
{
	register_native("give_weapon_coilgun", "native_give_weapon_add", 1)
}
...
{
	if(ItemID == g_Coilgun ) cs_set_user_bpammo(id, CSW_M249, BPAMMO)
}

public Mileage_WeaponRemove(id, ItemID)
{
	if(ItemID == g_Coilgun ) Remove_Coilgun(id)
}
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_valid_connected(invoker))
		return FMRES_IGNORED	
	if(get_user_weapon(invoker) != CSW_M249 || !Get_BitVar(g_Had_Coilgun, invoker))
		return FMRES_IGNORED
...
{
	if(!is_valid_connected(Attacker) || Victim == Attacker)
		return HAM_IGNORED	

	if(get_user_weapon(Attacker) != CSW_M249 || !Get_BitVar(g_Had_Coilgun, Attacker))
		return HAM_IGNORED
