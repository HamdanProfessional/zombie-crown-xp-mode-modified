...
// Zombie Attributes
new const zclass_name[] = "SirenZM" // name
...
new const zclass_screamsounds[][] = { "zombie_crown/siren/zc_sound_siren.wav" } // scream sound

// Scream ring color		R	G	B
new zclass_ring_colors[3] = {224,	47,	24}

// Class Powers
const zclass_health = 15000
const zclass_speed = 480
const Float:zclass_gravity = 0.56
const Float:zclass_knockback = 2.0
const zclass_level = 60
...
public plugin_init()
{
	...
	cvar_duration = register_cvar("zp_siren_scream_duration", "5")
...
public task_reload_scream(id)
{
	...
	// Message
	colored_print(id, GREEN, "[ZC]^x01 You can use again your scream^x04 power^x03 [E]")
	colored_print(id, GREEN, "[ZC]^x01 Remember, you can use your scream power pressing^x04 ^"+use^"^x01 button^x03 [E]")
}
...
public zp_user_infected_post(id, infector)
{
	...
	if(zp_get_user_zombie_class(id) == g_iSirenZID && !zp_get_zombie_hero(id))
	{
		// Array
		g_bCanDoScreams[id] = true
		
		// Message
		colored_print(id, GREEN, "[ZC]^x01 You can use^x04 screams^x01 pressing^x04 [E]^x01.")
	}
}
