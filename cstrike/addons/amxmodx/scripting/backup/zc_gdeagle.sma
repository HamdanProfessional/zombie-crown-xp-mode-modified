...
#define is_valid_player(%1) (1 <= %1 <= g_iMaxPlayers && is_user_connected(%1) && !is_user_hltv(%1) && !is_user_bot(%1))

new DG_V_MODEL[64] = "models/zombie_crown/v_golden_deagle.mdl"
new DG_P_MODEL[64] = "models/zombie_crown/p_golden_deagle.mdl"
...
// Sprite
new m_spriteTexture, g_iMaxPlayers
const Wep_deagle = ((1<<CSW_DEAGLE))

public plugin_init()
{
	register_plugin("[ZC GDG]", "1.1", "AlejandroSk")
	g_itemid = zp_register_extra_item("Golden Deagle", 200, ZP_TEAM_HUMAN, REST_MAP, 2)
	...
	cvar_dmgmultiplier = register_cvar("zp_goldendg_dmg_multiplier", "1.5")
	...
	register_concmd("amx_givegdeagle", "give_gun_gdeagle", ADMIN_LEVEL_C," <name or #userid>")
}
...
public give_gun_gdeagle(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))
	{
		...
		give_item(player, "weapon_deagle")
		g_HasDg[player] = true;
		client_print(id, print_console, "You gave to %s a Golden Deagle Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : GDEAGLE] --- [%s] - [%s]", admin_name, player_name);
	}else{
