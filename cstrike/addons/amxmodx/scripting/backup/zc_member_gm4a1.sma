...
new M4A1_V_MODEL[64] = "models/zombie_crown/v_golden_m4a1.mdl"
...
public plugin_init()
{
	register_plugin("[ZC Golden M4A1]", "1.1", "Bill=Thailand=[UP]")
	...
	g_itemid = zv_register_extra_item("Golden M4A1", 150, ZV_TEAM_HUMAN, REST_NONE, 2)
	g_itemid1 = zp_register_extra_item("Golden M4A1", 300, ZP_TEAM_HUMAN, REST_NONE, 2)
...
public give_gun_gm4a1(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))
	{
		...
		give_m4a1golden(player)
		client_print(id, print_console, "You gave to %s a Golden M4A1 Weapon.", player_name)
