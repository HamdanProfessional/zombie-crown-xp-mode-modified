...
public plugin_init()
{
	...
	register_concmd("amx_leave", "cmdLeave", ADMIN_RCON, "<tag> [tag] [tag] [tag]")
	register_concmd("amx_pause", "cmdPause", ADMIN_RCON, "- pause or unpause the game")
	register_concmd("amx_who", "cmdWho", ADMIN_ADMIN, "- displays who is on server")
	register_concmd("amx_cvar", "cmdCvar", ADMIN_RCON, "<cvar> [value]")
	...
	register_concmd("amx_cfg", "cmdCfg", ADMIN_RCON, "<filename>")
	register_concmd("amx_nick", "cmdNick", ADMIN_KICK, "<name or #userid> <new nick>")
...
public cmdSlap(id, level, cid)
{
	...
	if (damage < 0)
	{
		console_print(id, "[AMXX] Invalid command")
		return PLUGIN_HANDLED
	}
