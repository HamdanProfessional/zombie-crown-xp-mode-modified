...
new AK47_V_MODEL[64] = "models/zombie_crown/v_silver_ak47.mdl"
new AK47_P_MODEL[64] = "models/zombie_crown/p_silver_ak47.mdl"
...
public plugin_init()
{
	register_plugin("[ZC silver AK47]", "1.1", "n00bi2763")
	cvar_dmgmultiplier = register_cvar("zp_silverak_dmg_multiplier", "1.5")
	cvar_custommodel = register_cvar("zp_silverak_custom_model", "1")
	cvar_silverbullets = register_cvar("zp_silverak_silver_bullets", "1")
	cvar_uclip = register_cvar("zp_silverak_unlimited_clip", "1")
	g_itemid = zp_register_extra_item("Silver AK47", 100, ZP_TEAM_HUMAN, REST_NONE, 2)
	...
	register_concmd("amx_givesilverak", "give_gun_silverak", ADMIN_LEVEL_C," <name or #userid>")
}
...
public give_gun_gak47(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))
	{
		...
		give_AK47silver(player)
		client_print(id, print_console, "You gave to %s a silver AK47 Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : silver AK47] --- [%s] - [%s]", admin_name, player_name);
	}else{
