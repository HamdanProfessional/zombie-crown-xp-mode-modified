...
#define is_user_valid(%1) (1 <= %1 <= g_maxplayers && is_user_connected(id))
new g_at4cs, g_maxplayers
new g_had_at4cs[33], Float:g_lastfire[33], Float:g_lastaim[33], g_aiming[33],
g_smoke_id, g_spr_trail, g_spr_exp, is_reloading[33], cvar_radius, cvar_maxdamage

new const v_model[] = "models/zombie_crown/v_at4ex.mdl"
new const p_model[] = "models/zombie_crown/p_at4ex.mdl"
new const w_model[] = "models/zombie_crown/w_at4ex.mdl"
new const s_model[] = "models/zombie_crown/s_rocket.mdl"
...
public plugin_init() 
{
	...
	cvar_maxdamage = register_cvar("zp_at4cs_maxdamage", "900.0")
	g_at4cs = zv_register_extra_item("AT4CS", 100, ZV_TEAM_HUMAN, REST_NONE, 2)
	register_clcmd("weapon_at4cs", "hook_weapon")
	register_concmd("amx_giveat4cs", "giveat4cs", ADMIN_LEVEL_C," <name or #userid>")
...
{
	register_native("give_weapon_at4cs", "native_give_weapon_add", 1)
}
...
public giveat4cs(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))  
	{
		...
		g_had_at4cs[player] = 1
		is_reloading[player] = 0
		g_aiming[player] = 0
		fm_give_item(player, weapon_at4cs)
		static at4cs
		at4cs = fm_get_user_weapon_entity(player, CSW_AT4CS)
		cs_set_weapon_ammo(at4cs, 1)
		cs_set_user_bpammo(player, CSW_AT4CS, 10)
		client_print(id, print_console, "You gave to %s an AT4CS Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : AT4CS] --- [%s] - [%s]", admin_name, player_name);
	}else{
...
{
	if(!is_user_alive(id) || !is_user_connected(id) || !is_user_valid(id))
		return FMRES_IGNORED
	...
	if(CurButton & IN_ATTACK)
	{
		...
		if(cs_get_weapon_ammo(at4cs) > 0 && !is_reloading[id])
		{
			if(CurTime - 4.5 > g_lastfire[id])
			{
				...
				if(cs_get_user_zoom(id) && is_user_valid(id))
...
public fw_rocket_touch(rocket, touch)
{
	...
	while((iVictim = find_ent_in_sphere(iVictim, Origin, get_pcvar_float(cvar_radius))) != 0)
	{
		...
		&& iVictim != owner && zp_get_user_zombie(iVictim))
		{
			...
			ExecuteHamB(Ham_TakeDamage, iVictim, rocket, owner, Damage, DMG_BULLET)	
			new name[32]
			get_user_name(iVictim, name, 31);
			colored_print(owner, GREEN, "[ZC]^x01 Damage to ^x03%s^x01 :: ^x04%.1f damage", name, Damage)			
		}
	}	
