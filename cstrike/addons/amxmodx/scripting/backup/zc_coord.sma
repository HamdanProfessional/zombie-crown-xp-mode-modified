...
#define PLUGIN "[ZC SpawnPoint Maker]"
#define VERSION "1.0"
#define AUTHOR "Nguyen Son Tung"

new const BOXS_URL[] = "%s/zombie_crown/zc_others/zc_supplybox/%s.cfg"
const MAX_BOXS = 128
...
public plugin_init()
{
	...
	// Cmd
	register_concmd("zc_maker", "spawn_maker", ADMIN_RCON, "<make a spawn for prizes>");
}

public spawn_maker(id, level, cid)
{	
...
public box_main(id)
{
	...
	format(title, charsmax(title), "[ZC] Spawn Point Maker", LANG_PLAYER, "MENU_BOXS_TITLE", g_total_boxs, MAX_BOXS)
...
box_save()
{
	...
	// get url file
	new cfgdir[32], mapname[32], urlfile[368]
