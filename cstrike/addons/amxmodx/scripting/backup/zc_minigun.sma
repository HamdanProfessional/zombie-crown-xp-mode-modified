...
new Float:g_Time[33], g_M134

public plugin_init()
{
	register_plugin("[ZC] Minigun " , "1.0", "Remade by n00bi2763")
	...
	g_itemid = zp_register_extra_item("Minigun", 10, ZP_TEAM_HUMAN, REST_NONE, 2)
...
public give_minigun(player)
{
    // If the player already has the minigun, drop primary weapons.
    if (user_has_weapon(player, g_iWeaponID))
    {
        DropWeapons(player, DROP_PRIMARY);
    }
    
    // Set a flag (if needed) that indicates this player now has the minigun.
    g_pWeaponA[player] = true;
    
    // Give the minigun item.
    new weaponEntity = fm_give_item(player, WEAPON_BASE_NAME);
    if (weaponEntity > 0)
    {
        cs_set_weapon_ammo(weaponEntity, CLIP_WEAPON);
    }
    
    // Set the backup ammo for this weapon.
    cs_set_user_bpammo(player, g_iWeaponID, AMMO_WEAPON);
    
    // Update the player's weapon list.
    WeaponList(player, WEAPON_LIST, 3, AMMO_WEAPON, -1, -1, 0, 4, g_iWeaponID, 0);
}
