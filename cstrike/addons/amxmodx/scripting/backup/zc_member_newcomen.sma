...
new newcomen_V_MODEL[64] = "models/zombie_crown/v_spsmg.mdl"
new newcomen_P_MODEL[64] = "models/zombie_crown/p_spsmg.mdl"
new newcomen_W_MODEL[64] = "models/zombie_crown/w_spsmg.mdl"
...
new g_has_newcomen[33], g_clip_ammo[33], g_newcomen_TmpClip[33], oldweap[33], gmsgWeaponList
...
public plugin_init()
{
	register_plugin("[ZC Newcomen]", "1.0", "meNe")
	...
	cvar_dmg_newcomen = register_cvar("zp_newcomen_dmg", "1.9")
	...
	g_itemid_newcomen = zv_register_extra_item("NewComen", 160, ZV_TEAM_HUMAN, REST_MAP, 2)
	g_MaxPlayers = get_maxplayers()
	gmsgWeaponList = get_user_msgid("WeaponList")
	register_concmd("amx_givenewcomen", "give_gun_newcomen", ADMIN_LEVEL_C," <name or #userid>")
}
...
public give_gun_newcomen(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))  
	{
		...
		give_newcomen(player)
		client_print(id, print_console, "You gave to %s an NewComen Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : NEWCOMEN] --- [%s] - [%s]", admin_name, player_name);
	}else{
