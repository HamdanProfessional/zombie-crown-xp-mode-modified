...
new AK47_V_MODEL[64] = "models/zombie_crown/v_golden_ak47.mdl"
new AK47_P_MODEL[64] = "models/zombie_crown/p_golden_ak47.mdl"
...
public plugin_init()
{
	register_plugin("[ZC Golden AK47]", "1.1", "n00bi2763")
	...
	g_itemid = zv_register_extra_item("Golden AK47", 150, ZV_TEAM_HUMAN, REST_NONE, 2)
	g_itemid1 = zp_register_extra_item("Golden AK47 VIP", 300, ZP_TEAM_HUMAN, REST_NONE, 2)
	...
	register_concmd("amx_givegoldenak", "give_gun_goldenak", ADMIN_LEVEL_C," <name or #userid>")
}
...
public give_gun_gak47(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))
	{
		...
		give_AK47golden(player)
		client_print(id, print_console, "You gave to %s a Golden AK47 Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : GAK47] --- [%s] - [%s]", admin_name, player_name);
	}else{
