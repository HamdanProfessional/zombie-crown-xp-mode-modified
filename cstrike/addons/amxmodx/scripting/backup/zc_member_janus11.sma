...
new JANUS11_V_MODEL[64] = "models/zombie_crown/v_janus11.mdl"
new JANUS11_P_MODEL[64] = "models/zombie_crown/p_janus11.mdl"
new JANUS11_W_MODEL[64] = "models/zombie_crown/w_janus11.mdl"
...
new g_MaxPlayers, g_orig_event_janus11, g_IsInPrimaryAttack, g_janus11
new Float:cl_pushangle[33][3], m_iBlood[2], g_maxplayers
...
public plugin_init()
{
	register_plugin("[ZC Janus-11]", "1.0", "m4m3ts")
	...
	cvar_recoil_janus11 = register_cvar("zp_janus11_recoil", "0.65")           
	cvar_clip_janus11 = register_cvar("zp_janus11_clip", "15")
	cvar_janus11_ammo = register_cvar("zp_janus11_ammo", "64")
	cvar_dmg1_janus11 = register_cvar("zp_janus11_dmg1", "70.0")
	cvar_dmg2_janus11 = register_cvar("zp_janus11_dmg2", "140.0")
	cvar_transform_janus11 = register_cvar("zp_janus11_transform", "15")
	g_janus11 = zv_register_extra_item("Janus-11", 130, ZV_TEAM_HUMAN, REST_NONE, 2)
	g_MaxPlayers = get_maxplayers()
	register_concmd("amx_givejanus", "givejanus", ADMIN_LEVEL_C," <name or #userid>")
}
...
{
	register_native("give_weapon_janus", "native_give_weapon_janus", 1)
}	

public native_give_weapon_janus(id)
{
	if(!is_user_alive(id))
...
public givejanus(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))  
	{
		...
		give_janus11(player)
		client_print(id, print_console, "You gave to %s an Janus Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : JANUS] --- [%s] - [%s]", admin_name, player_name);
	}else{
...
{
	if (user_has_weapon(id, CSW_M3)) drop_weapons(id, 1)
