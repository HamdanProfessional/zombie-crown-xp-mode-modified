...
new g_iMaxPlayers, g_iEnemy[33], g_iInBlink[33], Float:g_fLastSlash[33], g_iCanceled[33], g_iSlash[33], g_iBlinks[33], g_itemid_blink

public plugin_init()
{
	register_plugin("[ZC KnifeB]", "1.0", "meNe");
	g_iMaxPlayers = get_maxplayers();
	g_itemid_blink = zp_register_extra_item("Knife Blink", 30, ZP_TEAM_ZOMBIE, REST_ROUND, 15)
...
public zp_extra_item_selected(player, itemid)
{
	if (itemid == g_itemid_blink)
	{
		...
		colored_print(player, GREEN, "[ZC]^x01 You have^x04 %d^x01 Knife Blinks", g_iBlinks[player]);
	}
...
public OP_SetBlink(id)
{
	...
	if (blink>1) return;
	if (blink) g_iInBlink[id] += 1;
}

public CHECK_ValidPlayer(id)
{
	if (1<=id<=g_iMaxPlayers && is_user_alive(id)) return 1;
	return 0;
}
