...
new ethereal_V_MODEL[64] = "models/zombie_crown/v_ethereal.mdl"
new ethereal_P_MODEL[64] = "models/zombie_crown/p_ethereal.mdl"
new ethereal_W_MODEL[64] = "models/zombie_crown/w_ethereal.mdl"
...
public plugin_init()
{
	register_plugin("[ZC Ethereal]", "1.0", "Zombie-Panic")
	...
	cvar_dmg_ethereal = register_cvar("zp_Ethereal_dmg", "12")
	cvar_clip_ethereal = register_cvar("zp_Ethereal_clip", "25")
	cvar_ethereal_ammo = register_cvar("zp_Ethereal_ammo", "100")
	g_itemid_ethereal = zv_register_extra_item("Ethereal", 250, ZV_TEAM_HUMAN, REST_NONE, 2)
	register_concmd("amx_giveethereal", "giveethereal", ADMIN_LEVEL_C," <name or #userid>")
...
public giveethereal(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))  
	{
		...
		Sprite(player)
		client_print(id, print_console, "You gave to %s an Ethereal Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : ETHEREAL] --- [%s] - [%s]", admin_name, player_name);
	}else{
...
{	new Player = get_pdata_cbase(Weapon, 41, 4)
	...
	if(Player > 0 && Player < 33)
	{
		...
		{
			if(szClip > 0)
				emit_sound(Player, CHAN_WEAPON, Fire_snd, VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
		}
		if(g_has_ethereal[Player])
		{
			new Float:push[3]
