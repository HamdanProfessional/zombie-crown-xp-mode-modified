...
public plugin_init()
{
	register_plugin("[ZC Tryder]", VERSION, "ILUSION")
	g_fumado = zp_register_extra_item("Tryder", 110, ZP_TEAM_HUMAN, REST_MAP, 3)
	...
	tryder_health = register_cvar("zp_tryder_health", "1500")
...
public zp_extra_item_selected(player, itemid)
{
	...
	if (itemid == g_fumado)
	{
		if(!zp_has_round_started() && zp_is_swarm_round() && zp_is_plague_round())
		{
...
{
	if (g_tryder[player] && !zp_get_user_first_zombie(player) && !zp_get_user_zombie(player) && !zp_get_zombie_hero(player) && !zp_get_human_hero(player))
	{
