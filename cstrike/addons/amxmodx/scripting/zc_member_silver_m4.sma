...
new M4A1_V_MODEL[64] = "models/zombie_crown/v_silver_m4a1.mdl"
...
public plugin_init()
{
	register_plugin("[ZC silver M4A1]", "1.1", "Bill=Thailand=[UP]")
	...
	g_itemid = zp_register_extra_item("Silver M4A1", 100, ZP_TEAM_HUMAN, REST_NONE, 2)
	g_itemid1 = zv_register_extra_item("Silver M4A1", 50, ZV_TEAM_HUMAN, REST_NONE, 2)
	...
	register_concmd("amx_givesm4a1", "give_gun_sm4a1", ADMIN_LEVEL_C," <name or #userid>")
}
...
public give_gun_gm4a1(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))
	{
		...
		give_m4a1silver(player)
		client_print(id, print_console, "You gave to %s a silver M4A1 Weapon.", player_name)
