...
#define PLUGIN "[ZC Dual Infinity]"
#define VERSION "1.0"
...
#define MODEL_V "models/zombie_crown/v_infinityex2.mdl"
#define MODEL_P "models/zombie_crown/p_infinityex2.mdl"
#define MODEL_W "models/zombie_crown/w_infinityex2.mdl"
...
public plugin_init()
{
	...
	g_Infinity = zp_register_extra_item("Dual Infinity", 100, ZP_TEAM_HUMAN, REST_MAP, 2)
...
public zp_extra_item_selected (id, item)
{
		...
		{
			colored_print(id, GREEN, "[ZC]^x01 You already have this^x04 item^x01.")
			return ZC_PLUGIN_HANDLED
		}else {
			Get_Infinity(id)
		}
    	}
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	...
	if(get_user_weapon(id) == CSW_INFINITY && Get_BitVar(g_Had_Infinity, id))
...
{
	if (!is_valid_connected(invoker))
		return FMRES_IGNORED	
	if(get_user_weapon(invoker) != CSW_INFINITY || !Get_BitVar(g_Had_Infinity, invoker))
		return FMRES_IGNORED
...
public fw_TraceAttack_World(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	if(!is_valid_connected(Attacker))
		return HAM_IGNORED	
	if(get_user_weapon(Attacker) != CSW_INFINITY || !Get_BitVar(g_Had_Infinity, Attacker))
		return HAM_IGNORED
