...
#define PLUGIN "[ZC PlasmaGun]"
#define VERSION "1.0"
#define AUTHOR "Dias Leon"

// Level 1 Config
#define V_MODEL "models/zombie_crown/v_plasma_rfl.mdl"
#define P_MODEL "models/zombie_crown/p_plasma_rfl.mdl"
#define W_MODEL "models/zombie_crown/w_primary.mdl"
...
public plugin_init() 
{
	...
	register_concmd("amx_givepg", "givepg", ADMIN_LEVEL_C," <name or #userid>")
	g_PlasmaGun = zv_register_extra_item("PlasmaGun", 170, ZV_TEAM_HUMAN, REST_NONE, 1)
}
...
public givepg(id, level, cid)
{
    	...
    	if(is_user_alive(player) && !zp_get_user_zombie(player) && !zp_get_human_hero(player) && !zp_get_zombie_hero(player))
	{
		...
		Get_Plasma(player)
		client_print(id, print_console, "You gave to %s a PlasmaGun Weapon.", player_name)
		log_to_file("zc_event.log", "[WEAPON EVENT : PLASMA] --- [%s] - [%s]", admin_name, player_name);
	}else{
...
public Get_Plasma(id)
{
	...
	if (user_has_weapon(id, CSW_AK47)) Remove_Plasma(id)
